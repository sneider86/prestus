...
<div class="wbcr-content-section">
	...
	<form method="post" action="" style="padding:20px;">
		<p>
			...
			<input type="text" id="wtitan-new-prefix" name="wtitan_new_prefix" value="<?php echo $data['random_prefix'] ?>">
		</p>
		<p>
		...
		<?php if( !is_null($data['random_prefix']) ): ?>
			<input type="hidden" name="wtitan_fixing_issue_id" value="<?php echo esc_attr($data['fixing_issue_id']) ?>">
		<?php endif; ?>
		<?php wp_nonce_field('wtitan_change_database_prefix'); ?>
		<p class="submit">
			<input type="submit" name="wtitan_save_prefix" class="button button-primary" value="<?php _e('Save Changes') ?>">
		</p>
	</form>
</div>